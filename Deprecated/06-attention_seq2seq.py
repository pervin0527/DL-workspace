...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs): ## hidden
        embedded = self.embedding(input).view(1, 1, -1)
        embedded = self.dropout(embedded)

        attn_weights = F.softmax(self.attn(torch.cat((embedded[0], hidden[0].view(1, 256)), 1)), dim=1)
...
if __name__ == "__main__":
    ...
    trainIters(encoder1, attn_decoder1, ITERATIONS, LEARING_RATE, MAX_LENGTH, TEACHER_FORCING_RATIO)
    evaluateRandomly(encoder1, attn_decoder1, MAX_LENGTH)