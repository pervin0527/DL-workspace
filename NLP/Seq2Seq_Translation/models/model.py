...
class EncoderBidGRU(nn.Module):
    def __init__(self, input_dim, embedd_dim, encoder_hidden_dim, decoder_hidden_dim, dropout):
        super().__init__()
        self.embedding = nn.Embedding(input_dim, embedd_dim)
        self.rnn = nn.GRU(embedd_dim, encoder_hidden_dim, bidirectional=True)
        self.fc = nn.Linear(encoder_hidden_dim * 2, decoder_hidden_dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, src):
        embedded = self.dropout(self.embedding(src))
        outputs, hidden = self.rnn(embedded) ## outputs : [src_len, batch_size, hidden_dim * num_directions], hidden : [num_layers * num_directions, batch_size, hidden_dim]
...
class AttentionDecoder(nn.Module):
    def __init__(self, output_dim, embedd_dim, encoder_hidden_dim, decoder_hidden_dim, dropout, attention):
        super().__init__()
        self.output_dim = output_dim
        self.attention = attention

        self.embedding = nn.Embedding(output_dim, embedd_dim)
        self.rnn = nn.GRU((encoder_hidden_dim * 2) + embedd_dim, decoder_hidden_dim)
        self.fc = nn.Linear((encoder_hidden_dim * 2) + decoder_hidden_dim + embedd_dim, output_dim)
        self.dropout = nn.Dropout(dropout)
