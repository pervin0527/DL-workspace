...
class YOLODetection(nn.Module):
    ...
    def forward(self, x, targets):
        """
        3개의 서로 다른 크기인 feature map에 3개의 앵커를 적용할 예정.
        각 head마다 한 번씩 실행되니까 결국 하나의 feature map에 3개의 앵커가 적용되는 상황이다.
        x : [batch_size, grid_size, grid_size, num_anchors * (5 + num_classes)]

        target : [image_idx_in_batch,  class_ids, x, y, w, h]
        ...
        pred : (모델이 예측한 offset만큼 anchor box를 조절한 prediction box, confidence score, classification scores)
        """
        ...
        cx = torch.sigmoid(prediction[..., 0])  # Center x(tx)
        cy = torch.sigmoid(prediction[..., 1])  # Center y(ty)
        w = prediction[..., 2]  # Width(tw)
        h = prediction[..., 3]  # Height(th)

        pred_conf = torch.sigmoid(prediction[..., 4])  ## conf score  [num_batches, num_anchors, grid_size, grid_size, 1]
        pred_cls = torch.sigmoid(prediction[..., 5:])  ## class score [num_batches, num_anchors, grid_size, grid_size, num_classes]

        ## 크기가 다른 feature map에 맞는 grid 정의.
        stride = self.image_size / grid_size ## 전체 downscale factor
        ...
        ## anchor의 너비와 높이를 grid scale로 조정한다.
        ...
        iou_scores, class_mask, obj_mask, no_obj_mask, tx, ty, tw, th, tcls, tconf = utils.utils.build_targets(pred_boxes=pred_boxes,
                                                                                                               pred_cls=pred_cls,
                                                                                                               target=targets,
