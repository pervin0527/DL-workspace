...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres, device):
    ...
    ## ground_truth box의 좌표를 grid scale에 맞도록 변환한다.
    target_boxes = target[:, 2:6] * nG
    gxy = target_boxes[:, :2] ## grid scale의 gt x, y, w, h
    gwh = target_boxes[:, 2:]

    ## feature map에 지정된 grid scale로 조절된 앵커박스와 gt 박스의 w, h를 이용해 IoU를 계산한다.
    ious = torch.stack([bbox_wh_iou(anchor, gwh) for anchor in anchors])
    _, best_ious_idx = ious.max(0)

    ## grid scale로 조절된 ground truth에서 필요한 값을 분리한다.
    b, target_labels = target[:, :2].long().t() ## image_idx, class_ids
    gx, gy = gxy.t() ## cx, cy
    gw, gh = gwh.t() ## w, h
    gi, gj = gxy.long().t() ## grid cell idx(i, j)

    ## ground truth와 scaled anchor box에서 iou가 가장 높은 앵커가 있는 곳에 object가 있음을 표기한다.
    obj_mask[b, best_ious_idx, gj, gi] = 1
    noobj_mask[b, best_ious_idx, gj, gi] = 0

    ## max_iou 위치는 아니지만, ignore_thres보다 높은 경우, object가 존재한다고 판단해 noobj_mask의 해당 위치를 False로 설정한다.
    for i, anchor_ious in enumerate(ious.t()):
        noobj_mask[b[i], anchor_ious > ignore_thres, gj[i], gi[i]] = 0

    ## i,j번째 grid cell 내부의 중심점이 얼마인지 계산한다. 3.5 - 3.0 = 0.5
    ...
    ## i, j번째 grid cell에 있는 객체의 클래스에 1로 표기.(one-hot encoding)
    tcls[b, best_ious_idx, gj, gi, target_labels] = 1

    ## 모델이 예측한 클래스와 정답 간의 비교 결과를 class_mask에 저장한다.
    ## 모델이 예측한 박스와 정답 간의 IoU 결과를 iou_scores에 저장한다.
